-- ██████████████████████████████████████████████████████████
-- ██  OMNI V305 — BLOXSTRIKE BYPASS (XENO/DELTA) PART 1    ██
-- ██████████████████████████████████████████████████████████
...
local Strings = {
    EN = {
        title = "OMNI V305",
        subtitle_mobile = "MOBILE · BLOXSTRIKE BYPASS",
        subtitle_pc = "UNIVERSAL · BLOXSTRIKE BYPASS",
        ...
        ntf_startup = "✅ Bloxstrike Bypass · Press RShift to Hide UI",
        ntf_lang = "Language changed to: ",
        stat_no_target = "No target", stat_auto_save = "⏱ Auto-save every 60s · OmniV305_BStrike.json",
    ...
    UA = {
        title = "OMNI V305", subtitle_mobile = "МОБІЛЬНА · BLOXSTRIKE ОБХОД",
        subtitle_pc = "УНІВЕРСАЛЬНА · BLOXSTRIKE ОБХОД",
        ...
        ntf_startup = "✅ Bloxstrike Bypass · Натисни RShift щоб приховати UI",
        ntf_lang = "Мову змінено на: ",
        stat_no_target = "Ціль відсутня", stat_auto_save = "⏱ Авто-зберігання кожні 60 сек · OmniV305_BStrike.json",
...
local CFG_FILE = "OmniV305_BStrike.json"
    ...
    while task.wait(0.5) do
        ...
        for _, p in pairs(Players:GetPlayers()) do
            ...
            for _, v in pairs(p.Character:GetDescendants()) do
                if v:IsA("BasePart") and not (v.Parent and (v.Parent:IsA("Accessory") or v.Parent:IsA("Hat"))) and v.Size.Magnitude > 0.3 and v.Size.X < s - 0.2 then
                    pcall(ApplyHB, v)
                end
            end
        end
...
local function DoPotato()
    ...
    for _, v in ipairs(all) do
        if v:IsA("ParticleEmitter") or v:IsA("Trail") or v:IsA("Beam") or v:IsA("BillboardGui") or v:IsA("SurfaceGui") or v:IsA("PointLight") or v:IsA("SpotLight") or v:IsA("SelectionBox") then
...
-- ============================================================
-- SILENT AIM HOOK (FIXED FOR BLOXSTRIKE)
-- ============================================================
    ...
    if ENV.hasHookMeta and hookmetamethod then
        local ok = pcall(function()
            hookmetamethod(game, "__namecall", newcclosure and newcclosure(function(self, ...)
                ...
                -- Фікс для Bloxstrike: перехоплюємо Raycast, FindPartOnRay, FindPartOnRayWithIgnoreList
                if (method == "Raycast" or method == "FindPartOnRay" or method == "FindPartOnRayWithIgnoreList") and self == Workspace then
            ...
            end) or function(self, ...) return self[method](self, ...) end)
        end)
        if ok then
            silentAimHooked = true
            Notify("Silent Aim", L("ntf_hook_ok") .. " [Meta]", 3)
            return
        end
    end
...
local serverActionCooldown = false local function ServerCooldown() if serverActionCooldown then Notify("Server Hop", L("ntf_wait"), 2); return true end serverActionCooldown = true task.delay(3, function() serverActionCooldown = false end) return false end
...
local function JoinRandomServer() if ServerCooldown() then return end Notify("Server Hop", L("ntf_search_rnd"), 3) task.spawn(function() local servers = GetServerList() if servers and #servers > 0 then local filtered = {} for _, s in pairs(servers) do if s.id ~= game.JobId and s.playing and s.playing > 0 then table.insert(filtered, s) end end if #filtered > 0 then pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, filtered[math.random(1, #filtered)].id, LP) end) return end end pcall(function() TeleportService:Teleport(game.PlaceId, LP) end) end) end
local function JoinBiggestServer() if ServerCooldown() then return end Notify("Server Hop", L("ntf_search_big"), 3) task.spawn(function() local servers = GetServerList() if servers and #servers > 0 then local best, bestCount = nil, -1 for _, s in pairs(servers) do if s.id ~= game.JobId and s.playing and s.playing > bestCount then bestCount = s.playing; best = s end end if best then Notify("Server Hop", "👥 " .. bestCount .. L("ntf_players"), 2) task.wait(1) pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, best.id, LP) end) return end end Notify("Server Hop", L("ntf_srv_fail"), 3) end) end
local function JoinSmallestServer() if ServerCooldown() then return end Notify("Server Hop", L("ntf_search_sml"), 3) task.spawn(function() local servers = GetServerList() if servers and #servers > 0 then local best, bestCount = nil, math.huge for _, s in pairs(servers) do if s.id ~= game.JobId and s.playing and s.playing < bestCount then bestCount = s.playing; best = s end end if best then Notify("Server Hop", "🕵️ " .. bestCount .. L("ntf_players"), 2) task.wait(1) pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, best.id, LP) end) return end end Notify("Server Hop", L("ntf_srv_fail"), 3) end) end

-- Чекаємо на "дай частину 2" для продовження коду...-- ============================================================
...
-- GHOST GUI HOOK: Ховає UI від античитів (Bloxstrike)
if ENV.hasHookMeta and hookmetamethod then
    pcall(function()
        local oldNC
        oldNC = hookmetamethod(game, "__namecall", newcclosure and newcclosure(function(self, ...)
            local method = getnamecallmethod and getnamecallmethod() or ""
            if (method == "GetChildren" or method == "GetDescendants" or method == "FindFirstChild") and (self == GuiP or self == Scr) then
                local res = oldNC(self, ...)
                if type(res) == "table" then
                    local filtered = {}
                    for _, v in ipairs(res) do
                        if v ~= Scr and not v:IsDescendantOf(Scr) then
            ...
            return oldNC(self, ...)
        end) or function(self, ...) return oldNC(self, ...) end)
    end)

    -- SPOOF WALKSPEED: Античит завжди бачить базову швидкість
    pcall(function()
        local oldIndex
        oldIndex = hookmetamethod(game, "__index", newcclosure and newcclosure(function(self, key)
            if not checkcaller() and self:IsA("Humanoid") and key == "WalkSpeed" and State.Speed then
                return gameBaseSpeed
            end
            return oldIndex(self, key)
        end) or function(self, key) return oldIndex(self, key) end)
    end)
end
